...
if "cluster" in job_properties:
    ...
    if "error" in cluster:
        args.e = cluster["error"]
    if "output" in cluster:
        args.o = cluster["output"]
...
if "resources" in job_properties:
    ...
    if "walltime_h" in resources:
        ...
        walltime="walltime=" + str(resources["runtime"] * 60)
...
# Make sure folders for stdout/stderr logs exist
if args.e:
    os.makedirs(os.path.dirname(args.e), exist_ok=True)
if args.o:
    os.makedirs(os.path.dirname(args.o), exist_ok=True)

cmd = "qsub {a}{A}{b}{c}{C}{d}{D}{e}{f}{h}{j}{l}{m}{M}{N}{o}{p}{P}{q}{t}{u}{v}{V}{w}{W}{rp}{dep}{ex}".format(a=atime,A=acc_string,b=pbs_time,c=chkpt,C=pref,d=dd,D=rd,e=se,f=ft,h=hold,j=j,l=resource,m=mail,M=mailuser,N=jname,o=so,p=priority,P=proxy,q=q,t=ar,u=user,v=ev,V=eall,w=wd,W=add,rp=resourceparams,dep=depend,ex=extras)

SUBMIT_ATTEMPTS = 1440
for i in range(SUBMIT_ATTEMPTS):
    try:
        res = subprocess.run(cmd, check=True, shell=True, stdout=subprocess.PIPE)
        break
    except subprocess.CalledProcessError as e:
        if i >= SUBMIT_ATTEMPTS - 1:
            print("This is the caught exception:", e, file = sys.stderr)
            raise e
        else:
            time.sleep(5 + 60 * i)
